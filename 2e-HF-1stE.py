...
The only difference is the coloumb integrals are more
complex and some numerical factors have changed
See paper for explaination
"""
import numpy as np
        ...
        for q in xrange(1,n_basis+1):
                for r in xrange(1,n_basis+1):
                        for s in xrange (1,n_basis+1):
                                newQ=0+0j
                                newQ+=FourierCp[0]*HF.I_gmn(0,p,q)*HF.I_gmn(-0,r,s)-1.0*FourierCp[0]*HF.I_gmn(0,p,s)*HF.I_gmn(-0,r,q)
                                pg=1
                                while pg <=30:
                                        newQ+=FourierCp[pg]*HF.I_gmn(pg,p,q)*HF.I_gmn(-pg,r,s)+FourierCn[pg]*HF.I_gmn(-pg,p,q)*HF.I_gmn(pg,r,s)-1.0*FourierCp[pg]*HF.I_gmn(pg,p,s)*HF.I_gmn(-pg,r,q)-1.0*FourierCn[pg]*HF.I_gmn(-pg,p,s)*HF.I_gmn(pg,r,q)
                                        pg+=1
...
for x1 in xrange(0,xsteps):
        for x2 in xrange(0,xsteps):
                PSI[x1,x2]=(psi1[x1]*psi2[x2]-psi1[x2]*psi2[x1])/np.sqrt(2)
