#!/usr/bin/env python
"""
Using the Hartree-Fock approximation to solve the problem
The only difference is the coloumb integrals are more complex and
some of the numerical factors have changed. See paper for explination
"""
import numpy as np
        ...
        for q in xrange(1,n_basis+1):
                for r in xrange(1,n_basis+1):
                        for s in xrange (1,n_basis+1):
                                newQ=0+0j
                                newQ+=FourierCp[0]*HF.I_gmn(0,p,q)*HF.I_gmn(-0,r,s)-0.5*FourierCp[0]*HF.I_gmn(0,p,s)*HF.I_gmn(-0,r,q)
                                pg=1
                                while pg <=30:
                                        newQ+=FourierCp[pg]*HF.I_gmn(pg,p,q)*HF.I_gmn(-pg,r,s)+FourierCn[pg]*HF.I_gmn(-pg,p,q)*HF.I_gmn(pg,r,s)-0.5*FourierCp[pg]*HF.I_gmn(pg,p,s)*HF.I_gmn(-pg,r,q)-0.5*FourierCn[pg]*HF.I_gmn(-pg,p,s)*HF.I_gmn(pg,r,q)
                                        pg+=1
                                Q[p-1,r-1,q-1,s-1]=(newQ/L*4)
print "Q calculated"
"""
...
for x1 in xrange(0,xsteps):
        for x2 in xrange(0,xsteps):
                PSI[x1,x2]=(psi1[x1]*psi2[x2]+psi1[x2]*psi2[x1])/np.sqrt(4)
...
for p in xrange(0,n_basis):
        for q in xrange(0,n_basis):
                Energy+=P[p,q]*T[p,q]/2
                Tenergy+=P[p,q]*T[p,q]
                for r in xrange(0,n_basis):
                        for s in xrange(0,n_basis):
                                Cenergy+=.5*P[p,q]*P[r,s]*Q[p,r,q,s]
Energy+=FockEnergy[0]
